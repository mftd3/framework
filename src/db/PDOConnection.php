...
use Exception;
use Generator;
use mftd\db\exception\BindParamException;
use mftd\db\exception\DbEventException;
use mftd\db\exception\DbException;
use mftd\db\exception\PDOException;
use mftd\Model;
use PDO;
use PDOStatement;
use Throwable;
...
abstract class PDOConnection extends Connection
{
    ...
    public function aggregate(BaseQuery $query, string $aggregate, $field, bool $force = false)
    {
        ...
        $field = $aggregate . '(' . (!empty($distinct) ? 'DISTINCT ' : '') . $this->builder->parseKey($query, $field, true) . ') AS mftd_' . strtolower($aggregate);
    ...
    {
        return $this->getConfig('builder') ?: '\\mftd\\db\\builder\\' . ucfirst($this->getConfig('type'));
    }
